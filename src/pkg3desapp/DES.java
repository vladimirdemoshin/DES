...
public class DES {
    ...
    public String encrypt(String input,String key)
    {
        ...
        for(int i=0;i<blocks.length;i++) 
        {
            ...
            blocks[i] = feistelNetwork(blocks[i],key,true); //полученный блок запускаем в 16-раундовую сеть Фейстеля
    ...
    {
        String output = "";
        String binaryCipher = toBinary(cipher); 
        String[] blocks = toBlocks(binaryCipher);
        for(int i=0;i<blocks.length;i++) 
        {
            blocks[i] = IP(blocks[i]); 
            blocks[i] = feistelNetwork(blocks[i],key,false);
            blocks[i] = lastTransformation(blocks[i]);
            output += binaryToString(blocks[i]);
        }
        return output;//toInitialLength(output);
    }
    ...
    public String toInitialLength(String input) //удаляет решётки в конце
    {
        int sharpIndex = 0;
        while(input.charAt(sharpIndex)!='#')
        {
            sharpIndex++;
            if(sharpIndex == input.length()) break;
        }
        return input.substring(0,sharpIndex);
    }
    ...
    public String feistelNetwork(String input,String key,boolean action) //true - шифруем, false - дешифруем
    {
        String T = input;
        ...
        int[] shiftDecryptTable = {0,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1};
        for(int i = 0; i < 16;i++)
        {
            if(action)
            {
            ...
            {
                 C = shift(C,shiftDecryptTable[i],false);
                 D = shift(D,shiftDecryptTable[i],false);
            }
            String roundKey = transformRoundKey(C+D);
            T = feistelRound(T,roundKey,action);
        }
        return T;
    }
    
    public String feistelRound(String T,String roundKey,boolean action)
    {
        ...
        else
        {
            String oldR = R;
            R = L;
            L = XOR(oldR,feistelEncodingFunction(L,roundKey));
        }
        return L + R;
    }
